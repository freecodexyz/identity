...
contract JsonClaimDifferential_T is Test {
    ...
    function testFuzz_AgreesOnClaimShapedInput(uint64 actorId, address wallet) public pure {
        string memory id = Strings.toString(uint256(actorId));

        bytes memory payload = abi.encodePacked(
            '{"jti":"0","sub":"repo:freecodexyz/identity:ref:refs/heads/main","aud":"',
            Strings.toHexString(uint160(wallet), 20),
            '","actor":"octocat","actor_id":"',
            id,
            '","repository_id":"1296269","event_name":"issues",',
            '"job_workflow_ref":"freecodexyz/identity/.github/workflows/register.yml@refs/heads/main"}'
        );

        _assertAgrees(payload, abi.encodePacked('"actor_id":"', id, '"'));
        _assertAgrees(payload, abi.encodePacked('"aud":"', Strings.toHexString(uint160(wallet), 20), '"'));
        _assertAgrees(payload, '"event_name":"issues"');
        _assertAgrees(
            payload, '"job_workflow_ref":"freecodexyz/identity/.github/workflows/register.yml@refs/heads/main"'
        );
